...
//
    ...
    func showConfirmDeactivateAccount() {
        showCustomAlertWithConfirm(title: deactiveAccountAlertTitle, message: deactiveAccountAlertMessage, vc: self) {
    ...
    func deactivateAccount() {
        ...
        params["user_email"] = txtLogin.text
        params["app-vers"] = PLUGIN_VERSION
        params["send_confirmation_email"] = "yes"
        
        
        let url = OTHER_API_PATH+"delete_account"
        
        CiyaShopAPISecurity.postAPICall2(url, dictParams: params){ (success, message, responseData) in
            let jsonReponse = JSON(responseData!)
            hideLoader()
            if let message = jsonReponse["message"].string {
                showCustomAlert(title: APP_NAME,message: message, vc: self, onConfirm: {
