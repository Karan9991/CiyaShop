...
class MCLocalization: NSObject {
    ...
    func preferredLanguage() -> String? {
        ...
        // this condition
        if let language = Bundle.preferredLocalizations(from: available, forPreferences: preferences).first , available.firstIndex(of: language) != nil {
            return language
        }
    ...
    func removeObserver(_ observer: MCLocalizationObserver) {
        let index = observers.firstIndex { (item) -> Bool in
