...
//MARK: - deactive account

var deactiveAccountAlertTitle = ""
var deactiveAccountAlertMessage = ""

func checkUserExist(vc : UIViewController){
    ...
    let url = OTHER_API_PATH+"is_user_exists"
    
    showLoader()
    CiyaShopAPISecurity.postAPICall(url, dictParams: params, completionBlock: {
        (success, message, responseData) in
        hideLoader()
        print( (success, message, responseData))
        let jsonReponse = JSON(responseData!)
        let userExists = jsonReponse["is_user_exists"].boolValue
        
        if !userExists && is_Logged_in {
            showCustomAlert(title: APP_NAME, message: getLocalizationString(key: "logoutNotExistUser"), vc: vc) {
                removeLoginData()
            }
